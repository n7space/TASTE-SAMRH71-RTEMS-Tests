...
def test_samrh71_rtems_passthrough():
    remote_gdb_server = os.getenv("SAMRH71_REMOTE_GDBSERVER", default="127.0.0.1")
    ser = serial.Serial("SAMRH71", baudrate=9600, timeout=1)

    build = common.do_clean_build(
        "samrh71-rtems-passthrough/test-samrh71-passthrough-sender"
    )
    build = common.do_build(
        "samrh71-rtems-passthrough/test-samrh71-passthrough-sender", ["samrh71", "debug"]
    )
    ...
    try:
        ...
        gdbmi.write(
            "file samrh71-rtems-passthrough/test-samrh71-passthrough-sender/work/binaries/partition_1"
        )
    ...
    common.do_clean_build("samrh71-rtems-passthrough/test-samrh71-passthrough-receiver")
    remote_gdb_server = os.getenv("SAMRH71_REMOTE_GDBSERVER", default="127.0.0.1")

    build = common.do_build(
        "samrh71-rtems-passthrough/test-samrh71-passthrough-receiver", ["samrh71", "debug"]
    )
    stderr = build.stderr.decode("utf-8")
    assert build.returncode == 0, f"Compilation errors: \n{stderr}"
    ...
    common.run_verification_project(
        remote_gdb_server,
        "samrh71-rtems-passthrough/test-samrh71-passthrough-receiver/work/binaries/partition_1",
        "receiver.c",
        "24",
    )
    stop_sender.set()
...
if __name__ == "__main__":
    test_samrh71_rtems_passthrough()
