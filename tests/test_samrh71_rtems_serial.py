...
def test_samrh71_rtems_serial():
    remote_gdb_server = os.getenv("SAMRH71_REMOTE_GDBSERVER", default="127.0.0.1")
    build = common.do_build("samrh71-rtems-serial", ["samrh71", "debug"])
    ...
    try:
        ...
        gdbmi.write("file samrh71-rtems-serial/work/binaries/partition_1")
        ...
        errors = common.do_execute(
            "samrh71-rtems-serial", expected, test_exe="work/binaries/partition_2"
        )
    finally:
        gdbmi.exit()
    assert not errors, "\n".join(errors)
